...
/*   Updated: 2025/07/05 18:35:34 by mel-houa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pipe_check(t_token *tokenze)
{
	...
	while (tmp && tmp->next)
	{
		if (tmp->type == PIPE && tmp->next->type == PIPE)
			return ((free_token(tokenze)), printf("PIPE ERROR\n"), 0);
		tmp = tmp->next;
	}
...
int	red_check(t_token *tokenze)
{
	...
	while (tmp)
	{
		...
		if (tmp->type == REDIR_IN && (tmp->next->type == REDIR_OUT
				|| tmp->next->type == REDIR_IN))
			return ((free_token(tokenze)),
				(printf("syntax error near unexpected token `>'\n")), 0);
		if (tmp->type == REDIR_OUT && (tmp->next->type == REDIR_OUT
				|| tmp->next->type == REDIR_IN))
			return ((free_token(tokenze)),
				(printf("syntax error near unexpected token `<'\n")), 0);
		tmp = tmp->next;
